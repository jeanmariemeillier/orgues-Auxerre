<div id="accueil">
   <section>
   <article id="menu_accueil">
   <ul>
      <li>
      ANCIENS INSTRUMENTS
      </li>
      <li>
         Les Frères Précheurs
      </li>
      <li>
         Notre-Dame-la-d'Hors
      </li>
      <li>
      EGLISES ACTUELLES
      </li>
      <li>
      Saint-Eus�be l'orgue de choeur
      </li>
      <li>
      Saint-Eus�be le grand-orgue
      </li>
      <li></li>
      <li></li>
   </ul>
   

   </article>
   </section>
   <section>
      <figure>
         <img src="images/orgue_puzzle_poster.jpg" alt="Puzzle unique de l'orgue de la cathédraled'Auxerre" title="Puzzle unique de l'orgue de la cathédrale d'Auxerre">
         <figcaption>Puzzle unique de l'orgue de la cathédrale d'Auxerre</figcaption>
      </figure>   
   </section>
   <section>
   <article id="Actu">
        <p>ACTUALITES</p>
        <div id="ActuContenant"> 
          <div id="ActuTexte"> 
            <p> <b>Avancement des recherches</b> <br />
            </p>
            <p> En ce qui concerne les orgues des �glises d'Auxerre, aucune recherche 
              compl�te n'a �t� faite. <BR>
              Sur cette pr�sentation, les titres en bleu sont des pages de ce 
              site, les titres en gris n'ouvrent pas de pages. </p>
            <p><b>La cath�drale</b> <br />
              L'avant-derni�re ligne en bleu pr�sente les orgues de la cath�drale 
              ; initialement pr�vu comme un site � part enti�re celui-ci a �t� 
              incorpor� dans l'histoire des orgues d'Auxerre. Cela devenait l'Arl�sienne, 
              ce dont on parle et qu'on ne voit jamais. </p>
            <p> <b>LECTURE DU SITE</b> <BR>
              Il est pr�f�rable � l'ouverture du site, de visionner les pages 
              dans cet ordre : <BR>
              Le site <BR>
              L'auteur <BR>
              Pr�sentation <BR>
              Certaines explications permettent de mieux comprendre ce qui suit, 
              mais sans obligation ! </p>
          </div>
        </div>
      </article>
      <article id="txt_accueil">
      <p>
         Ceci n'est pas un site web, du moins, pas encore ; <font color="#FF0000">c'est un feuilleton</font>, dont chacun des �pisodes est complet (sauf d�couvertes nouvelles).
      </p>
      <p>
		 Certaines des donn�es qui le composent ou le composeront sont sur deux blogs int�ressant les Orgues de l'Yonne.
      </p>
      </article>
   </section>
</div>